...
PRODCOST_BY_INTERVAL(INTERVAL) =E=
                ...
                )+ DH_LOSS_LOAD(INTERVAL)*4000*INTERVAL_MINUTES(INTERVAL)/60
                 + LOSS_LOAD(INTERVAL)*SYSTEMVALUE("VOLL")*INTERVAL_MINUTES(INTERVAL)/60*SYSTEMVALUE("MVA_PERUNIT") + ADDITIONAL_LOAD_SLACK(INTERVAL)*SYSTEMVALUE("VOLL")*INTERVAL_MINUTES(INTERVAL)/60*SYSTEMVALUE("MVA_PERUNIT")
                 + SUM(RESERVETYPE,INSUFFICIENT_RESERVE(INTERVAL,RESERVETYPE)*RESERVEVALUE(RESERVETYPE,"VOIR")*INTERVAL_MINUTES(INTERVAL)/60*SYSTEMVALUE("MVA_PERUNIT"))
$IFI '%NETWORK_CHECK%' == YES                  + SUM(BRANCH, PHASE_SHIFTER_ANGLE1(BRANCH,INTERVAL)*.005 + PHASE_SHIFTER_ANGLE2(BRANCH,INTERVAL)*.005)
$IFI '%NETWORK_CHECK%' == YES                  + SUM(BRANCH,(BRANCH_SLACK1(BRANCH,INTERVAL)+BRANCH_SLACK2(BRANCH,INTERVAL))*SYSTEMVALUE("VOLL")*INTERVAL_LENGTH*SYSTEMVALUE("MVA_PERUNIT"))
...
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES           =L= INTERVAL_LENGTH*60*STORAGEVALUE(CONVENTIONAL_STORAGE_GEN,"PUMP_RAMP_RATE")*
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES          (PUMPING(CONVENTIONAL_STORAGE_GEN,INTERVAL) - (PREVIOUS_UNIT_PUMPUP(CONVENTIONAL_STORAGE_GEN)
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES           + UNIT_PUMPUP(CONVENTIONAL_STORAGE_GEN,INTERVAL))) +
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES          (PREVIOUS_UNIT_PUMPUP(CONVENTIONAL_STORAGE_GEN) + UNIT_PUMPUP(CONVENTIONAL_STORAGE_GEN,INTERVAL))*
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES           INTERVAL_LENGTH*STORAGEVALUE(CONVENTIONAL_STORAGE_GEN,"MIN_PUMP")/STORAGEVALUE(CONVENTIONAL_STORAGE_GEN,"PUMP_STARTUP_TIME")
...
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES          - (SUM(H$(ORD(H) GE ORD(INTERVAL) AND (ORD(H) LE (ORD(INTERVAL) + PUMPDOWN_PERIOD(CONVENTIONAL_STORAGE_GEN)-1))),UNIT_PUMPDOWN(CONVENTIONAL_STORAGE_GEN,H))*
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES          STORAGEVALUE(CONVENTIONAL_STORAGE_GEN,"MIN_PUMP")*(INTERVAL_LENGTH/STORAGEVALUE(CONVENTIONAL_STORAGE_GEN,"PUMP_SHUTDOWN_TIME")))
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES          - GEN_FORCED_OUT(CONVENTIONAL_STORAGE_GEN)*STORAGEVALUE(CONVENTIONAL_STORAGE_GEN,"MAX_PUMP")
$IFI '%SUSD_TRAJECTORY_CHECK%' == YES ;
...
(NUCLEAR_RAMP_UP(NUCLEAR_GEN,INTERVAL)-NUCLEAR_RAMP_UP(NUCLEAR_GEN,INTERVAL-1))*(GENVALUE(NUCLEAR_GEN,"HOLD_TIME")-1)=l=
...
$offText
* Added to fix the circular non-confirmity
Q_MIN_STABLE_FIRST_HOUR(NUCLEAR_GEN,INTERVAL)$( (GENVALUE(NUCLEAR_GEN,"INITIAL_STABLE_HOUR") LT GENVALUE(NUCLEAR_GEN,"HOLD_TIME")) AND(ORD(INTERVAL) EQ 1) )..
(GEN_SCHEDULE(NUCLEAR_GEN,INTERVAL)-GENVALUE(NUCLEAR_GEN,"INITIAL_MW"))=E=0;
*******************************************#####################################
Q_CONSECUTIVE_RAMP_UP_DN(NUCLEAR_GEN,INTERVAL)$(ORD(INTERVAL) LT (CARD(INTERVAL)- GENVALUE(NUCLEAR_GEN,"HOLD_TIME")) )..
NUCLEAR_RAMP_DN(NUCLEAR_GEN,INTERVAL)+SUM(H$ ( ( ORD(H) GT ORD(INTERVAL)) AND(ORD(H) LT (ORD(INTERVAL)+GENVALUE(NUCLEAR_GEN,"HOLD_TIME"))) ),NUCLEAR_RAMP_UP(NUCLEAR_GEN,H) )=l=1;
********************************************####################################
...
*****************************Reserve_remodeling*********************************
$onText
Q_GENLIMIT_LOW_NUC_MIN(NUCLEAR_GEN,INTERVAL)..
GEN_SCHEDULE(NUCLEAR_GEN,INTERVAL) =G= UNIT_STATUS(NUCLEAR_GEN,INTERVAL)*GENVALUE(NUCLEAR_GEN,"MIN_GEN")
*$onText
         +SUM(RESERVETYPE$(RESERVEVALUE(RESERVETYPE,"RESERVE_ON")EQ 1 AND (RESERVEVALUE(RESERVETYPE,"RESERVE_DIR") EQ 2 OR RESERVEVALUE(RESERVETYPE,"RESERVE_DIR") EQ 3)),
         GEN_RESERVE_SCHEDULE(NUCLEAR_GEN,INTERVAL,RESERVETYPE))
...
;
********************************************Reserve affected by xenon-poisoning**********************
